...
class AdminController extends Controller
{
    ...
    public function AdminFilterCategoryReports(Request $request){
        ...
        ";
            ...
            if($query != '')
            {
                ...
                ->whereDate('archives.created_at', [$date1, $date2])
               ->get();
            ...
            if($total_row > 0)
            {
                ...
                $TotalRanks = DB::table('archives')
                ->select('category', DB::raw('SUM(count_rank) as totalrank'))
                ->where('category', $category)
                ->whereBetween('created_at', [$date1, $date2])
                ->groupBy('category')
                ->get();
    ...
    public function AdminSearchStudentReports(Request $request){
        ...
        $students = studentModel::select('student_models')->distinct()->get();
        $systeminformation = SystemInformation::all();
        return view('admin.studentreportview', compact('systeminformation','student_models'));
